...
namespace WindowsFormsApp1
{
    ...
    public partial class StakeholderMaintenanceForm : Form
    {
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            // Updated signature: Assuming _dbManager.AddStakeholder no longer takes a password argument
            int newId = _dbManager.AddStakeholder(
            ...
            if (newId > 0)
            {
                MessageBox.Show($"Stakeholder '{txtEmail.Text}' added successfully! ID: {newId}", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
                LoadStakeholders();
                ClearInputFields();
            }
            else if (newId == -2)
            {
                MessageBox.Show("Failed to add stakeholder: The email address is already in use.", "Error: Duplicate Email", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            else
            {
