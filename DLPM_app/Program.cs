...
namespace DLPM
{
    ...
    internal class DBManager
    {
        ...
        /// Accounts for the consolidated Stakeholder table.
        /// </summary>
        public List<ParticipantAssessmentStatusModel> GetParticipantAssessmentStatus(int participantStakeholderID)
        {
            ...
            // SQL: Retrieves all assessments, joins to Modules, joins to the Partner (Stakeholder) that owns the module.
            // Then LEFT JOINs to Enrollment and ModuleAssessmentResult for the current participant's status.
            string sql = @"
                SELECT 
                    MA.AssessmentID, 
                    MA.AssessmentName,
                    ...
                    M.ModuleID, 
                    M.ModuleName,
                    P.PartnerName, -- Module owner's name from Stakeholder table
                    
                    E.EnrollmentID, -- Check for enrollment status
                    
                    MAR.Mark AS LastScore, 
                    MAR.AssessmentDate AS LastAttemptDate -- Get the latest result
                FROM ModuleAssessment MA
                INNER JOIN Module M ON MA.ModuleID = M.ModuleID
                -- Join to the Partner (Stakeholder) who owns the module. 
                -- Assuming Module.StakeholderID links to the Partner's StakeholderID.
                INNER JOIN Stakeholder P ON M.StakeholderID = P.StakeholderID 
                -- Check if the participant is enrolled in the module (LEFT JOIN allows showing non-enrolled modules)
                LEFT JOIN Enrollment E ON M.ModuleID = E.ModuleID AND E.StakeholderID = @ParticipantID
                -- Get the latest assessment result for this participant/module pair
                LEFT JOIN (
                    SELECT 
                        MAR_Inner.*,
                        ROW_NUMBER() OVER (PARTITION BY MAR_Inner.EnrollmentID, MAR_Inner.AssessmentID ORDER BY MAR_Inner.AssessmentDate DESC) as rn
                    FROM ModuleAssessmentResult MAR_Inner
                ) MAR ON E.EnrollmentID = MAR.EnrollmentID AND MA.AssessmentID = MAR.AssessmentID AND MAR.rn = 1
                ORDER BY M.ModuleName, MA.AssessmentName;";
            ...
            try
            {
                using (SqlConnection connection = new SqlConnection(_connectionString))
                {
                    ...
                    using (SqlCommand command = new SqlCommand(sql, connection))
                    {
                        command.Parameters.AddWithValue("@ParticipantID", participantStakeholderID);

                        using (SqlDataReader reader = command.ExecuteReader())
                        {
                            while (reader.Read())
                            {
                                // Get nullable fields safely
                                int enrollmentIDOrdinal = reader.GetOrdinal("EnrollmentID");
                                int lastScoreOrdinal = reader.GetOrdinal("LastScore");
                                int lastAttemptDateOrdinal = reader.GetOrdinal("LastAttemptDate");

                                bool isEnrolled = !reader.IsDBNull(enrollmentIDOrdinal);
                                decimal? lastScore = reader.IsDBNull(lastScoreOrdinal) ? (decimal?)null : reader.GetDecimal(lastScoreOrdinal);
                                DateTime? lastDate = reader.IsDBNull(lastAttemptDateOrdinal) ? (DateTime?)null : reader.GetDateTime(lastAttemptDateOrdinal);
                                ...
                                statuses.Add(new ParticipantAssessmentStatusModel
                                {
                                    ...
                                    HasTakenAssessment = lastDate.HasValue,
                                    LastScore = lastScore,
            ...
            {
                Console.WriteLine($"\n[DB ERROR - GetParticipantAssessmentStatus] Failed to fetch data: {ex.Message}");
            }
        ...
        public bool DeleteModule(int moduleID)
        {
            ...
            catch (SqlException sqlex) when (sqlex.Number == 547)
            {
                ...
                Console.WriteLine($"\n[DB ERROR] Cannot delete module due to foreign key constraint: {sqlex.Message}");
                return false;
            }
            catch (Exception ex)
            {
                Console.WriteLine($"\n[DB ERROR] Error deleting module: {ex.Message}");
                return false;
            }
        }
        ...
        /// </summary>
        public int AddStakeholder(string firstName, string lastName, string email, string password, string roleType, string partnerName = null, string contactNumber = null)
        {
            // In a real application, 'password' would be the hashed value. For consistency, we use it as the simple value here.
            string sql = @"
                INSERT INTO Stakeholder (FirstName, LastName, EmailAddress, PasswordHash, RoleType, PartnerName, ContactNumber) 
                VALUES (@FirstName, @LastName, @Email, @Password, @RoleType, @PartnerName, @ContactNumber); 
                SELECT SCOPE_IDENTITY();";
            ...
            {
                Console.WriteLine($"\n[DB ERROR] Error adding stakeholder: Email address already exists.");
                return -2; // Custom error code for duplicate email
            }
            catch (Exception ex)
            {
                Console.WriteLine($"\n[DB ERROR] Error adding stakeholder: {ex.Message}");
                return -1; // Indicate generic failure
            }
        }
        ...
        public List<ParticipantPerformanceSummary> GetParticipantPerformanceSummary(int? moduleId, int? partnerId)
        {
            ...
            // This complex query uses CTEs (Common Table Expressions) to first calculate scores 
            // and then aggregate them to a participant level.
            string sql = $@"
            WITH ParticipantScores AS (
                SELECT
                    ...
                    AR.Score,
                    A.PassPercentage,
                    CASE WHEN AR.Score >= A.PassPercentage THEN 1 ELSE 0 END AS Passed
                FROM Stakeholder S
                JOIN AssessmentResult AR ON S.StakeholderID = AR.StakeholderID
                JOIN Assessment A ON AR.AssessmentID = A.AssessmentID
                WHERE S.RoleType = 'Participant'
                {(moduleId.HasValue ? " AND A.ModuleID = @ModuleID" : "")}
                {(partnerId.HasValue ? " AND A.PartnerID = @PartnerID" : "")}
            )
            SELECT
                ParticipantName,
                IDNumber,
                COUNT(StakeholderID) AS AssessmentsTaken,
                CAST(AVG(Score) AS DECIMAL(5,2)) AS AverageScorePercentage,
                    ...
                    using (SqlCommand command = new SqlCommand(sql, connection))
                    {
                        ...
                        using (SqlDataReader reader = command.ExecuteReader())
                        {
                            while (reader.Read())
                            {
                                ...
                                if (taken > 0)
                                {
                                    // Calculate pass rate percentage for status determination
        ...
        /// <summary>
        /// Retrieves summary data for all Assessments, filtered by an optional date range.
        /// </summary>
        public List<AssessmentModuleSummary> GetAssessmentSummaryByModule(DateTime? startDate, DateTime? endDate)
        {
            ...
            // Calculates the average score and pass rate for each assessment
            string sql = $@"
            WITH AssessmentStats AS (
                SELECT
                    ...
                    AR.Score,
                    A.PassPercentage,
                    CASE WHEN AR.Score >= A.PassPercentage THEN 1 ELSE 0 END AS IsPassed
                FROM Assessment A
                JOIN Module M ON A.ModuleID = M.ModuleID
                LEFT JOIN AssessmentResult AR ON A.AssessmentID = AR.AssessmentID
                WHERE 1=1
                {(startDate.HasValue ? " AND AR.DateTaken >= @StartDate" : "")}
                {(endDate.HasValue ? " AND AR.DateTaken <= @EndDate" : "")}
            )
            SELECT
                ModuleName,
                AssessmentName,
                ...
                ISNULL(CAST(SUM(IsPassed) * 100.0 / NULLIF(COUNT(Score), 0) AS DECIMAL(5,2)), 0) AS PassRatePercentage
            FROM AssessmentStats
            GROUP BY ModuleName, AssessmentName, DateCreated
            ORDER BY ModuleName, AssessmentName;";
            ...
            try
            {
                using (SqlConnection connection = new SqlConnection(_connectionString))
                {
                    ...
                    using (SqlCommand command = new SqlCommand(sql, connection))
                    {
                        ...
                        if (endDate.HasValue) command.Parameters.AddWithValue("@EndDate", endDate.Value.Date.AddDays(1).AddSeconds(-1)); // End of day
