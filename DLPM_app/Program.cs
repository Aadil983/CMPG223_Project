...
namespace DLPM
{
    ...
    internal class DBManager
    {
        ...
        /// <summary>
        /// Data model used for the Participant Dashboard grid to show available assessments 
        /// and the participant's last score/status for each.
        /// </summary>
        public List<AvailableAssessment> GetAvailableAssessmentsForParticipant(int participantId)
        {
            ...
            // Query finds all Assessments linked via Enrollment and gets the latest result (if any)
            string sql = @"
        SELECT 
            A.AssessmentID, 
            A.AssessmentName, 
        ...
        WHERE E.StakeholderID = @ParticipantID
        JOIN Module M ON A.ModuleID = M.ModuleID
        JOIN CommunityPartner CP ON A.PartnerID = CP.PartnerID
        GROUP BY A.AssessmentID, A.AssessmentName, M.ModuleName, CP.PartnerName, A.PassPercentage
                ...
                using (SqlConnection connection = new SqlConnection(_connectionString))
                {
                    ...
                    using (SqlCommand command = new SqlCommand(sql, connection))
                    {
                        ...
                        using (SqlDataReader reader = command.ExecuteReader())
                        {
                            while (reader.Read())
                            {
                                DateTime? lastDate = reader["LastAttemptDate"] is DBNull ? (DateTime?)null : reader.GetDateTime(reader.GetOrdinal("LastAttemptDate"));
                                decimal lastScore = reader["LastScore"] is DBNull ? 0m : reader.GetDecimal(reader.GetOrdinal("LastScore"));
